...
impl EventQueue {
    ...
    pub fn peek(&self, current_time: Instant) -> (Option<&SimEvent>, Queue, Duration) {
        match self.len() {
            ...
            _ => {
                ...
                let (mut first, mut q) = (self.base.peek(), Queue::Base);
                ...
                let duration_since = first.unwrap().time.duration_since(current_time);
    ...
    pub fn peek_non_blocking(&self) -> (Option<&SimEvent>, Queue) {
...
fn before(a: Option<&SimEvent>, b: Option<&SimEvent>) -> bool {
    match (a, b) {
        (Some(a), Some(b)) => {
            let a_time = a.time;
            let b_time = b.time;
