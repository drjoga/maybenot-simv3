...
fn main() {
    ...
    match &cli.command {
        Commands::CreateTracebinHi {
            ...
            create_tracebin_hi(bw_tracefile, save_file, sizebins, binpktsizes);
        }
        Commands::CreateTracebinStd {
            bw_tracefile,
            save_file,
        } => {
            create_tracebin_std(bw_tracefile, save_file);
        }
...
fn check_trace_length(trace: &LinkTrace, scalefactor: usize) {
    let bw_len = trace.bw_trace.len();
    if bw_len < 1_000 * scalefactor {
        println!(
            "Warning: The trace has a short length < 1 sec. Now it is : {:.3} sec",
            bw_len as f64 / (1e3_f64 * scalefactor as f64)
        );
    }
    if bw_len > 100000 * scalefactor {
        println!(
...
fn create_tracebin_hi(bw_tracefile: &str, save_file: &str, sizebins: &str, binpktsizes: &str) {
    if !bw_tracefile.ends_with(".tr") && !bw_tracefile.ends_with(".tr.gz") {
        panic!("The tracefile must end with .tr or .tr.gz");
    }

    println!(
        "Creating simplex trace binary file with tracefile: {}, sizebins: {}, binpktsizes: {}",
        bw_tracefile, sizebins, binpktsizes
    );
    ...
    let linktrace = LinkTrace::new_hi_res(bw_tracefile, sizebin_lookuptable);
...
fn create_tracebin_std(bw_tracefile: &str, save_file: &str) {
    if !bw_tracefile.ends_with(".tr") && !bw_tracefile.ends_with(".tr.gz") {
        panic!("The tracefile must end with .tr or .tr.gz");
    }

    println!(
        "Creating simplex trace binary file with tracefile: {}",
        bw_tracefile
    );

    let linktrace = LinkTrace::new_std_res(bw_tracefile);
